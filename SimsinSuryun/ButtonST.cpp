...
CButtonST::CButtonST()
{
	...
	m_RoundRate = 0;		// 모서리 Round 비율    [2005-03-11-HJH(ADD)]
	m_nTextAlign = DT_CENTER;		// [2005-07-28-HJH(ADD)]
} // End of CButtonST
...
//=================================================================================================//
// CButtonST::SetTextAlign       Date : 2005-07-28, Author: 한준호
// Explain	  : 버튼에 그려질 Text위치 조정 
// Parameters : DrawText() Format Flags
// Return	  : 
//=================================================================================================//
void CButtonST::SetTextAlign(int nTextAlign)
{
...
//		  pDC->Draw3dRect(itemRect, ::GetSysColor(COLOR_BTNSHADOW), ::GetSysColor(COLOR_BTNHILIGHT));
	    ...
	    CPen penBtnHiLight(PS_SOLID, 1, GetSysColor(COLOR_BTNHILIGHT)); // Bianco
        CPen penBtnShadow(PS_SOLID, 1, GetSysColor(COLOR_BTNSHADOW));   // Grigio scuro
...
//*/
        ...
        pDC->MoveTo( itemRect.left	+ m_RoundRate	, itemRect.bottom-1 );

		// left - bottom 모서리 
		pDC->ArcTo(  itemRect.left	, itemRect.bottom - m_RoundRate*2	, itemRect.left + m_RoundRate*2		, itemRect.bottom	, 
					 itemRect.left	+ m_RoundRate	, itemRect.bottom	, itemRect.left	, itemRect.bottom	- m_RoundRate		);

		// left line
		pDC->LineTo( itemRect.left	, itemRect.top		+ m_RoundRate );
		
		// left - top 모서리 
		pDC->ArcTo(  itemRect.left	, itemRect.top	, itemRect.left + m_RoundRate*2	, itemRect.top + m_RoundRate*2	, 
					 itemRect.left	, itemRect.top	+ m_RoundRate	, itemRect.left	+ m_RoundRate	, itemRect.top	);

		// top line
        pDC->LineTo( itemRect.right - m_RoundRate	, itemRect.top );

		// right - top 모서리 
		pDC->ArcTo(  itemRect.right	- m_RoundRate*2	, itemRect.top	, itemRect.right, itemRect.top + m_RoundRate*2	, 
					 itemRect.right - m_RoundRate	, itemRect.top	, itemRect.right, itemRect.top + m_RoundRate	);

		// 회색선으로 바꿔줌 
		pDC->SelectObject(penBtnHiLight);

		// right line
        pDC->LineTo( itemRect.right	, itemRect.bottom - m_RoundRate );

		// right - bottom 모서리 
		pDC->ArcTo(  itemRect.right		- m_RoundRate*2				, itemRect.bottom - m_RoundRate*2		, itemRect.right, itemRect.bottom	, 
					 itemRect.right	, itemRect.bottom - m_RoundRate , itemRect.right - m_RoundRate			, itemRect.bottom );

		// bottom line
        pDC->LineTo( itemRect.left	+ m_RoundRate	, itemRect.bottom-1 );
...
//*/
  ...
  {
    CPen penBtnHiLight(PS_SOLID, 1, GetSysColor(COLOR_BTNHILIGHT)); // White
    CPen penBtnShadow(PS_SOLID , 1, GetSysColor(COLOR_BTNSHADOW));   // Dark gray
...
//	Rgn.CreateRoundRectRgn(0,0,rect1.Width(),rect1.Height(), 30,30);
//	VERIFY(pDC->FillRgn( &Rgn, &brA));      // rgnA Red Filled
//	GetDlgItem(IDC_LOGIN)->SetWindowRgn(Rgn, TRUE);
//	Rgn.DeleteObject();
...
DWORD CButtonST::SetBk(CDC* pDC)
{
	if (m_bDrawTransparent && pDC)
	{
		...
		GetWindowRect(rect1);GetParent()->ScreenToClient(rect1);
